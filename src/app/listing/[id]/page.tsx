...
import { MapPin, Home, Star, IndianRupee, BadgeCheck, Calendar, User, Check } from "lucide-react";
...
export default async function PropertyPage({ params }: PropertyPageProps) {
  ...
  const mediaItems = [
    ...
    ...(property.sampleFlatVideo ? [{ type: 'video', url: property.sampleFlatVideo, title: 'Sample Flat' }] : []),
    ...(property.localityVideo ? [{ type: 'video', url: property.localityVideo, title: 'Locality' }] : [])
  ];
        ...
        <div className="flex flex-col gap-8 mb-8">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-1/3 space-y-6">
              ...
              <div className="p-6 bg-purple-50 dark:bg-purple-900/20 rounded-xl shadow-sm border border-purple-100 dark:border-purple-900/30">
                <h2 className="text-xl font-semibold mb-4 text-purple-700 dark:text-purple-200">Property Highlights</h2>
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="text-sm text-gray-500 dark:text-gray-400">BHK</p>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{property.seller?.name || "Not available"}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {property.amenities?.length > 0 && (
            <div className="p-6 bg-green-50 dark:bg-green-900/20 rounded-xl shadow-sm border border-green-100 dark:border-green-900/30">
              <h2 className="text-xl font-semibold mb-4 text-green-700 dark:text-green-200">Amenities</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {property.amenities.map((amenity, index) => (
                  <div key={index} className="flex items-center">
                    <Check className="w-4 h-4 mr-2 text-green-500" />
                    <span className="text-gray-700 dark:text-gray-300">{amenity.trim()}</span>
                  </div>
                ))}
              </div>
            </div>
